...
const ClickToDefense = ({ onTime, handleTimer, isDone ,handleResult}: onTimeProp) => {
  ...
  return (
    <div>
      ...
      <h3>RESULT</h3>
      {onTime  ? <div>{clicked}번이나 스페이스바가 무리하는 중...😥</div> : <></>}
      {!onTime && !isDone ? <div>대기시간 후 게임이 시작됩니다</div>:<></>}
      {isDone ? (isSuccess ? <div>성공!! {end}번이나 하다니!!😀</div>:<div>실패.. {end}번을 못하네ㅋㅋ🤣</div>): <></>}
